import { clusterPermission } from "@/pages/utils"
...
export const canViewNavMenuGroup = item => {
  ...
  if (permission && permission.menu) {
    return item.children.some(el => permission.menu.find(u => u.name === el.title)?.view)
  }

  return false
}

export const canViewNavMenu = item => {
  const permission = clusterPermission()
  ...
  if (permission && permission.menu) {
    return permission.menu.find(u => u.name === item.title)?.view
  }
